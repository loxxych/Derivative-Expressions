...
Expression<T> Parser<T>::parseFactor()
{
    ...
    if (match(Number))
    {
        ...
        if (lexeme == "i") {
            ...
            // not complex
            return Expression<T>(static_cast<T>(std::stod(lexeme)));
        }
    }
